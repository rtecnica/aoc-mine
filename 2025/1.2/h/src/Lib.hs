...
transitState (state, count) move = (new_state, new_count)
    ...
    new_count = count + div (mag move) 100 + extras
    extras = if (state_change || new_state == 0) && state /= 0 
            then 1
            else 0
    state_change = case dir move of
        Lib.Left -> state < new_state
        Lib.Right -> state > new_state
